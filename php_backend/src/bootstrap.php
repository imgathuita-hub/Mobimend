...
// Prefer Composer's autoloader when vendor/ exists (local dev & production).
// Falls back to the custom spl_autoload_register so the app still works
// if Composer hasn't been run yet.
...
set_exception_handler(static function (Throwable $exception): void {
    ...
    if (PHP_SAPI !== 'cli') {
        ...
        echo '</head><body><div class="card"><h1>Mobimend Error</h1>';
        echo '<p>' . htmlspecialchars($message, ENT_QUOTES, 'UTF-8') . '</p>';

        if ($debug) {
            echo '<pre>' . htmlspecialchars($exception->getTraceAsString(), ENT_QUOTES, 'UTF-8') . '</pre>';
        } else {
            echo '<p>Set APP_DEBUG=true in php_backend/.env to see full details.</p>';
        }
